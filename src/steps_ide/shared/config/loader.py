...
class ConfigLoader:
    ...
    def _get_config_dir(self) -> Path:
        ...
        # Use XDG standard: ~/.config/steps_ide/
        config_home = os.environ.get('XDG_CONFIG_HOME')
        if config_home:
            return Path(config_home) / 'steps_ide'
        return Path.home() / '.config' / 'steps_ide'
    ...
    def _get_default_config_dir(self) -> Path:
        ...
        if getattr(sys, 'frozen', False):
            ...
            return base_path / 'steps_ide' / 'resources' / 'default_configs'
        else:
    ...
    def get_icon_path(self) -> Optional[Path]:
        ...
        if sys.platform == 'win32':
            icon_name = 'steps_ide.ico'
        else:
            icon_name = 'steps_ide.png'

        # Get the resources directory (handles both dev and bundled)
        if getattr(sys, 'frozen', False):
            base_path = Path(sys._MEIPASS)
            icons_dir = base_path / 'steps_ide' / 'resources' / 'icons'
        else:
